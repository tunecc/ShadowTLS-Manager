...
install_tools() {
    local missing_tools=()
    for tool in wget curl openssl jq systemd-detect-virt; do
...
# 域名校验：仅验证域名是否有效（利用 nslookup 检查解析情况）
...
# 
# 这里对 create_service 函数进行改动，检测宿主机是否为 LXC 或 KVM 并根据需要保留或去掉 ExecStartPre
#
create_service() {
    ...
    # 检测宿主机虚拟化类型
    # LXC 或 KVM，由 systemd-detect-virt 返回值决定
    local virt_type
    ...
    # 准备 service 文件时，根据虚拟化类型决定是否写入 ExecStartPre
    # 当系统架构是LXC => 不写 ExecStartPre
    # 当系统架构是KVM => 写入 ExecStartPre
...
EOF
    ...
    if [[ "$virt_type" == "kvm" ]]; then
        echo 'ExecStartPre=/bin/sh -c "ulimit -n 51200"' >> "$SERVICE_FILE"
        print_info "检测到KVM虚拟化环境，已添加 ExecStartPre 配置以提高性能"
    else
        print_info "检测到${virt_type}虚拟化环境，不需要添加 ExecStartPre 配置"
    fi
...
uninstall_shadowtls() {
    ...
    if [[ "${confirm,,}" == "y" ]]; then
        ...
        # 删除定时重启任务
        print_info "检查并删除定时重启任务..."
        if crontab -l 2>/dev/null | grep -q "systemctl restart shadow-tls"; then
            crontab -l 2>/dev/null | grep -v "systemctl restart shadow-tls" | crontab -
            print_info "已删除 Shadow-TLS 的定时重启任务"
        fi
...
# ===========================
# 新增：设置每天定时重启的功能（通过 cron 实现）
schedule_daily_restart() {
    local hour
    local minute

    # 输入小时
    read -rp "请输入每天定时重启的小时 (0-23): " hour
    if ! [[ "$hour" =~ ^[0-9]+$ ]] || [ "$hour" -lt 0 ] || [ "$hour" -gt 23 ]; then
        print_error "输入的小时格式不正确，请输入0-23之间的整数。"
        return
    fi

    # 输入分钟
    read -rp "请输入每天定时重启的分钟 (0-59): " minute
    if ! [[ "$minute" =~ ^[0-9]+$ ]] || [ "$minute" -lt 0 ] || [ "$minute" -gt 59 ]; then
        print_error "输入的分钟格式不正确，请输入0-59之间的整数。"
        return
    fi

    (crontab -l 2>/dev/null | grep -v 'systemctl restart shadow-tls'; echo "$minute $hour * * * systemctl restart shadow-tls") | crontab -
    print_info "已设置每天 $hour 点 $minute 分 自动重启 Shadow-TLS 服务。"
}
...
main_menu() {
    while true; do
        ...
        if [[ -e /usr/local/bin/shadow-tls ]]; then
            if systemctl is-active --quiet shadow-tls; then
                status_text="${Green_font_prefix}已安装并运行中${RESET}"
            else
                status_text="${Yellow_font_prefix}已安装但未运行${RESET}"
            fi
        else
            status_text="${Red_font_prefix}未安装${RESET}"
        fi
        
        echo -e "\n${Cyan_font_prefix}=== Shadow-TLS 管理脚本 ===${RESET}"
        echo -e "\n${White_font_prefix}当前状态: ${RESET}$status_text\n"
        
        echo -e "${Cyan_font_prefix}■ 安装与更新${RESET}"
        echo -e "  ${Green_font_prefix}1.${RESET} 安装 Shadow-TLS"
        echo -e "  ${Green_font_prefix}2.${RESET} 升级 Shadow-TLS"
        echo -e "  ${Green_font_prefix}3.${RESET} 卸载 Shadow-TLS"
        
        echo -e "\n${Cyan_font_prefix}■ 配置管理${RESET}"
        echo -e "  ${Green_font_prefix}4.${RESET} 查看配置信息"
        echo -e "  ${Green_font_prefix}5.${RESET} 修改配置"
        
        echo -e "\n${Cyan_font_prefix}■ 服务控制${RESET}"
        echo -e "  ${Green_font_prefix}6.${RESET} 启动服务"
        echo -e "  ${Green_font_prefix}7.${RESET} 停止服务"
        echo -e "  ${Green_font_prefix}8.${RESET} 重启服务"
        
        echo -e "\n${Cyan_font_prefix}■ 其他${RESET}"
        echo -e "  ${Green_font_prefix}9.${RESET} 设置定时重启"
        echo -e "  ${Green_font_prefix}0.${RESET} 退出脚本"
        echo ""
        ...
        case "$choice" in
            ...
            7) stop_service ;;
            8) restart_service ;;
            9) schedule_daily_restart ;;
            0) 
                echo -e "\n${Yellow_font_prefix}已退出 Shadow-TLS 管理脚本${RESET}"
                exit 0 
                ;;
            *) 
                echo -e "\n${Red_font_prefix}错误: 请输入有效的选项 [0-9]${RESET}" 
                sleep 2
                ;;
        esac
        echo -e "\n按任意键返回主菜单..."
